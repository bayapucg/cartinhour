...
public function search()
    
  {
             ...
             $match = $this->input->post('search');
                ...
                $result1 = $this->closingfee_model->search($match);
                $result2 = count($result1);
                //echo $result2;exit;
              $this->load->library('pagination');

              $config = [
               'base_url'       =>  base_url('admin/closingfee/search'),
               'per_page'       =>  5,
               'total_rows'     =>  $result2,
               'full_tag_open'  =>  "<ul class='pagination'>",
               'full_tag_close' =>  "</ul>",
             ...
             $this->pagination->initialize($config);
            //$this->load->view('welcome_message');
            $result=$this->closingfee_model->limit($config['per_page'], $this->uri->segment(4) )->search($match);
            $data['closingdata'] =  $result;
            $this->template->write_view('content', 'admin/closingfee/index',$data);
