...
public function search()
    
  {
             ...
             $match = $this->input->post('search');
                ...
                $result1 = $this->shipping_model->search();
                $result2 = count($result1);
                //echo $result2;exit;
              $this->load->library('pagination');

              $config = [
               'base_url'       =>  base_url('admin/shipping/search'),
               'per_page'       =>  5,
               'total_rows'     =>  $result2,
               'full_tag_open'  =>  "<ul class='pagination'>",
               'full_tag_close' =>  "</ul>",
             ...
             $this->pagination->initialize($config);
            //$this->load->view('welcome_message');
            $result=$this->shipping_model->limit($config['per_page'], $this->uri->segment(4) )->search($match);
            $data['cihdata'] =  $result;
            $this->template->write_view('content', 'admin/shipping/index',$data);
